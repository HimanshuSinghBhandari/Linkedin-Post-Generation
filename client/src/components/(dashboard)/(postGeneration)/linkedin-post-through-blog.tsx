...
const BlogUrlPostGenerator: React.FC = () => {
  ...
  const handleGeneratePost = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post('http://localhost:3002/scrape', { url: blogUrl });
      ...
      const generatedPost = await generateLinkedInThroughBlogPost(scrapedContent, finalOptions);
      ...
      <div className="relative z-10">
        ...
        >
          <motion.h1
            initial={{ y: -20 }}
            animate={{ y: 0 }}
            className="text-2xl sm:text-4xl font-extrabold mb-6 text-center text-zinc-100"
          >
            Blog URL Post Generator
          </motion.h1>
          ...
          <motion.div
            ...
            className="space-y-3"
          >
            <label className="font-semibold text-zinc-300 text-sm sm:text-base">Blog URL</label>
            <input
              type="text"
              className="w-full p-3 sm:p-4 rounded-lg bg-gray-700 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-teal-500 text-xs sm:text-base"
                ...
                <div className="relative">
                  <Listbox value={selectedOptions[postOption.label] || ''} onChange={(value) => handleOptionChange(postOption.label, value)}>
                    <div className="relative mt-1">
                      <Listbox.Button className="relative w-full cursor-default rounded-lg bg-gray-700 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-teal-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 text-xs sm:text-base">
                  ...
                  <input
                    type="text"
                    className="w-full p-2 mt-2 rounded-lg bg-gray-700 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-teal-500 text-xs sm:text-base"
          ...
          {generatedContent && (
            <motion.div
              ...
              className="mt-8"
            >
              <h2 className="font-semibold mb-4 text-lg sm:text-xl text-teal-400">Generated Content:</h2>
              <LinkedInPostCard
...
export default BlogUrlPostGenerator;